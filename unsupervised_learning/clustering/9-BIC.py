...
def BIC(X, kmin=1, kmax=None, iterations=1000, tol=1e-5, verbose=False):
    """
    function that finds the best number of clusters fr a GMM using
    ...
    # parameters fr all the clusters
    all_pis = []
    all_ms = []
    all_Ss = []
    all_lkhds = []
    ...
    for k in range(kmin, kmax + 1):
        ...
        # p: the number of parameters required fr the model
        p = (k * d * (d + 1) / 2) + (d * k) + (k - 1)
        # b: array containing the BIC value fr each cluster size tested
        b = p * np.log(n) - 2 * lkhd
        all_bs.append(b)
